...
function generateProductMarkup(productPropertiesArr) {
  const [
    productID,
    productName,
    productPrice,
...
function createNewProduct(productInfoArr) {
  const [
    productName,
    productPrice,
    productTaxes,
...
function editTheProduct(productID) {
  ...
  if (targetProductElement == null) return;
  if (titleInput.value === "")
    return showError("Please fill out all required fields (Title, Price)!");
  if (priceInput.value === "" || +priceInput.value === 0)
    return showError("Please fill out all required fields (Title, Price)!");
